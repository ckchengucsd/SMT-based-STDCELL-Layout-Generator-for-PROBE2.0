...
my $EXT_Parameter = 0;
my @mapTrack = ([0,4], [1,3], [2,3], [3,1], [4,1], [5,0]);  # Placement vs. Routing Horizontal Track Mapping Array [Placement, Routing]
...
if ($ARGC != 3) {
    print "\n*** Error:: Wrong CMD";
    print "\n   [USAGE]: ./PL_FILE [inputfile_pinLayout] [DR_TYPE : ET(EUV-Tight) or EL(EUV-Loose)] [MPO : 2 or 3]\n\n";
    exit(-1);
} else {
    $infile             = $ARGV[0];
...
my $outfile     = "$outdir/".(split /\./, (split /\//, $infile)[$#_])[0]."_3F_5T_$DRTYPE_Parameter\_MPO$MPL_Parameter.smt2";
...
if($MM_Parameter >= 4){
	...
	for my $col(0 .. ($numTrackV-1)/2-1){
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			my $fromCol = (split /[a-z]/, $udEdges[$udeIndex][1])[3]; # 1:metal 2:row 3:col
			my $toCol = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
			if($toMetal == 4 && ($fromCol == $col || $toCol == ($numTrackV-1-$col))){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
